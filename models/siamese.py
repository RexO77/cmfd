...
class SiameseNetwork(nn.Module):
    def __init__(self, feature_dim=768, hidden_dims=[512, 256, 128], dropout_rates=[0.3, 0.3, 0.2]):
        ...
        # Enhanced final classification layer
        # Remove sigmoid since we'll use BCEWithLogitsLoss
    ...
    def forward(self, feat1, feat2):
        ...
        if self.compute_l2_distance:
            l2_distance = torch.sqrt(torch.sum((f1_att - f2_att)**2, dim=1, keepdim=True).expand(-1, f1_att.size(1)))
        ...
        # Pass through classifier network (without sigmoid)
        return self.fc(x)
