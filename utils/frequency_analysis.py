...
class FrequencyAnalyzer:
    ...
    def __init__(
        ...
        mid_freq_focus=(1, 32),
        histogram_bins=50,
        noise_sampling_size=100
    ):
        """
        Initialize frequency analyzer with configurable parameters.
        
        Args:
            dct_threshold: Threshold for DCT coefficient similarity (0-1)
    ...
    def _compare_dct(self, patch1, patch2):
        ...
        corr = np.corrcoef(dct1_norm, dct2_norm)[0, 1]
    ...
    def _compare_noise_residuals(self, patch1, patch2):
        ...
        corr_hist = np.corrcoef(hist1, hist2)[0, 1]
        ...
        if min_samples >= 100:
            ...
            corr_patterns = np.corrcoef(samples1, samples2)[0, 1]
    ...
    def analyze_frequency_coherence(self, image, suspicious_pairs, patch_size=64):
        ...
        corr = np.corrcoef(avg_src_dct, avg_dst_dct)[0, 1]
        if np.isnan(corr):
            corr = 0.0
    ...
    def analyze_frequency_bands(self, image, suspicious_pairs, patch_size=64, num_bands=4):
        """
        Analyze multiple frequency bands in DCT domain for suspicious pairs
        
        This provides more detailed frequency analysis by dividing the DCT 
        coefficients into multiple bands (low, mid-low, mid-high, high)
        
        Args:
            image: Input image (BGR or grayscale)
            suspicious_pairs: List of ((x1, y1), (x2, y2)) coordinate pairs
            patch_size: Size of patches to analyze
            num_bands: Number of frequency bands to analyze (2-4)
            
        Returns:
            Dictionary with frequency band analysis results for each suspicious pair
        """
        if len(suspicious_pairs) == 0:
            return {'band_similarities': [], 'band_visualization': None}
        ...
        # Define frequency bands (adjust ranges based on patch size)
        # For 64x64 patch, coefficients range from [0,0] to [63,63]
        max_freq = patch_size - 1
        
        # Create frequency bands based on specified number of bands
        bands = []
        if num_bands == 2:
            # Simple low/high split
            bands = [
                {'name': 'low', 'range': (0, max_freq//2)},
                {'name': 'high', 'range': (max_freq//2+1, max_freq)}
            ]
        elif num_bands == 3:
            # Low/mid/high split
            bands = [
                {'name': 'low', 'range': (0, max_freq//3)},
                {'name': 'mid', 'range': (max_freq//3+1, 2*max_freq//3)},
                {'name': 'high', 'range': (2*max_freq//3+1, max_freq)}
            ]
        else:  # num_bands == 4 or more
            # Low/mid-low/mid-high/high split
            bands = [
                {'name': 'low', 'range': (0, max_freq//4)},
                {'name': 'mid-low', 'range': (max_freq//4+1, max_freq//2)},
                {'name': 'mid-high', 'range': (max_freq//2+1, 3*max_freq//4)},
                {'name': 'high', 'range': (3*max_freq//4+1, max_freq)}
            ]
            bands = bands[:num_bands]  # Truncate if requested bands < 4
            
        # Initialize results for each pair and band
        band_similarities = []
        ...
        for (x1, y1), (x2, y2) in suspicious_pairs:
            ...
            # Skip if patches are outside image bounds
            if src_patch.shape != (patch_size, patch_size) or dst_patch.shape != (patch_size, patch_size):
                band_similarities.append({band['name']: 0.0 for band in bands})
                continue
            ...
            for band in bands:
                ...
                # Extract band coefficients (using zigzag pattern for better coverage)
                band_coeffs1 = []
                band_coeffs2 = []
                ...
                for i in range(patch_size):
                    for j in range(patch_size):
                        ...
                        # This gives us radial frequency bands rather than rectangular
                        freq = np.sqrt(i*i + j*j)
                        if start <= freq <= end:
                            band_coeffs1.append(dct1[i, j])
                            band_coeffs2.append(dct2[i, j])
                
                # Convert to numpy arrays for correlation
                band_coeffs1 = np.array(band_coeffs1)
                band_coeffs2 = np.array(band_coeffs2)
                
                # Skip if insufficient coefficients
                if len(band_coeffs1) < 5:
                    pair_results[band_name] = 0.0
                    continue
                    
                # Normalize coefficients
                band_coeffs1_norm = (band_coeffs1 - np.mean(band_coeffs1)) / (np.std(band_coeffs1) + 1e-10)
                band_coeffs2_norm = (band_coeffs2 - np.mean(band_coeffs2)) / (np.std(band_coeffs2) + 1e-10)
                
                # Calculate correlation
                corr = np.corrcoef(band_coeffs1_norm, band_coeffs2_norm)[0, 1]
                if np.isnan(corr):
                    corr = 0.0
                    
                # Convert correlation to similarity (0-1)
                similarity = 0.5 * (corr + 1.0)
                pair_results[band_name] = similarity
                
            # Save results for this pair
            band_similarities.append(pair_results)
            
        # Create visualization showing differences across bands
        band_visualization = self._create_band_visualization(image, suspicious_pairs, 
                                                        band_similarities, bands, patch_size)
            
        return {
            'band_similarities': band_similarities,
            'band_visualization': band_visualization
        }
